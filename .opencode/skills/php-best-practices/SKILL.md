...
## Project Override — foundry-stack

> **Read before applying any rule.** In foundry-stack, PHP 8.4 property hooks and asymmetric visibility are NOT adopted — see `.opencode/skills/laravel-foundry/SKILL.md` anti-patterns section. Skip `modern-property-hooks` and `modern-asymmetric-visibility` rules. The project uses PHP `^8.4 || ^8.5`, Laravel 13, PHPStan max level, and a modular monolith with hexagonal Core — not a standalone `App\` app.
