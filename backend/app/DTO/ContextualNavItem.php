...
final class ContextualNavItem
{
    ...
    public static function validate(array $config): array
    {
        ...
        $hasTitle = isset($config['title']) && is_string($config['title']) && $config['title'] !== '';
        $hasTitleTemplate = isset($config['title_template']) && is_string($config['title_template']) && $config['title_template'] !== '';
        if (!$hasTitle && !$hasTitleTemplate) {
            $errors[] = "Falta 'title' o 'title_template'";
        }

        // route_name o route_name_suffix
        $hasRouteName = isset($config['route_name']) && is_string($config['route_name']) && $config['route_name'] !== '';
        $hasRouteSuffix = isset($config['route_name_suffix']) && is_string($config['route_name_suffix']) && $config['route_name_suffix'] !== '';
        ...
        // current
        if (isset($config['current']) && !is_bool($config['current'])) {
